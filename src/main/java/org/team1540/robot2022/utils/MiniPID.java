...
/**
 * Source: https://github.com/tekdemo/MiniPID-Java Small, easy to use PID implementation with advanced controller capability.<br> Minimal usage:<br> MiniPID pid = new MiniPID(p,i,d); <br> ...looping
 * code...{ <br> output= pid.getOutput(sensorvalue,target); <br> }
 */
public class MiniPID {
    ...
    /**
     * Configure the Proportional gain parameter. <br> This responds quickly to changes in setpoint, and provides most of the initial driving force to make corrections. <br> Some systems can be used
     * with only a P gain, and many can be operated with only PI.<br> For position based controllers, this is the first parameter to tune, with I second. <br> For rate controlled systems, this is
     * often the second after F.
     *
    ...
    /**
     * Changes the I parameter <br> This is used for overcoming disturbances, and ensuring that the controller always gets to the control mode. Typically tuned second for "Position" based modes, and
     * third for "Rate" or continuous based modes. <br> Affects output through <b>output+=previous_errors*Igain ;previous_errors+=current_error</b>
     *
    ...
    /**
     * Changes the D parameter <br> This has two primary effects:
     * <list>
     ...
     * </list>
     * A small D value can be useful for both improving response times, and preventing overshoot. However, in many systems a large D value will cause significant instability, particularly for large
     * setpoint changes.
     * <br>
    ...
    /**
     * Configure the FeedForward parameter. <br> This is excellent for velocity, rate, and other  continuous control modes where you can expect a rough output value based solely on the setpoint.<br>
     * Should not be used in "position" based control modes.<br> Affects output according to <b>output+=F*Setpoint</b>. Note, that a F-only system is actually open loop.
     *
    ...
    /**
     * Specify a maximum output range. <br> When one input is specified, output range is configured to
     * <b>[-output, output]</b>
     */
    ...
    /**
     * Specify a  maximum output. When two inputs specified, output range is configured to
     * <b>[minimum, maximum]</b>
     *
    ...
    /**
     * Configure setpoint for the PID calculations<br> This represents the target for the PID system's, such as a position, velocity, or angle. <br>
     */
    ...
    /**
     * Calculate the output value for the current PID cycle.<br> In no-parameter mode, this uses the last sensor value, and last setpoint value. <br> Not typically useful, and use of parameter modes
     * is suggested. <br>
     *
     * @return calculated output value for driving the system
     */
    ...
    /**
     * Calculate the output value for the current PID cycle.<br> In one parameter mode, the last configured setpoint will be used.<br>
     *
     * @param actual The monitored value, typically as a sensor input.
     * @return calculated output value for driving the system
     */
    ...
    /**
     * Resets the controller. This erases the I term buildup, and removes D gain on the next loop.<br> This should be used any time the PID is disabled or inactive for extended duration, and the
    ...
    /**
     * Set the maximum rate the output can increase per cycle.<br> This can prevent sharp jumps in output when changing setpoints or enabling a PID system, which might cause stress on physical or
     * electrical systems.  <br> Can be very useful for fast-reacting control loops, such as ones with large P or D values and feed-forward systems.
     *
    ...
    /**
     * Set a filter on the output to reduce sharp oscillations. <br> 0.1 is likely a sane starting value. Larger values use historical data more heavily, with low values weigh newer data. 0 will
     * disable, filtering, and use only the most recent value. <br> Increasing the filter strength will P and D oscillations, but force larger I values and increase I term overshoot.<br> Uses an
     * exponential wieghted rolling sum filter, according to a simple <br>
    ...
    /**
     * To operate correctly, all PID parameters require the same sign This should align with the {@literal}reversed value
     */
