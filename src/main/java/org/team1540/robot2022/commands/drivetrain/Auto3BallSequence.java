...
public class Auto3BallSequence extends SequentialCommandGroup {
    ...
    public Auto3BallSequence(Drivetrain drivetrain, Intake intake, Indexer indexer, Shooter shooter, Hood hood, Limelight limelight, RepeatCommand indexCommand) {
        addCommands(
                deadline( // End this command when the path sequence is done
                        sequence(                          // Run the path sequence
                                ...
                                new ShootSequence(shooter, indexer, drivetrain, hood, intake, limelight, indexCommand)      // Shoot the 1 indexed balls (collects)
                        ),
                        sequence(
