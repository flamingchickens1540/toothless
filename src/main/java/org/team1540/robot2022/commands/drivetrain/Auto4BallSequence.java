...
public class Auto4BallSequence extends SequentialCommandGroup {
    ...
    public Auto4BallSequence(Drivetrain drivetrain, Intake intake, Indexer indexer, Shooter shooter, Limelight limelight, RepeatCommand indexCommand, InterpolationTable interpolationTable) {
        addCommands( 
            deadline( // End this command when the path sequence is done
                sequence (                          // Run the path sequence
                    ...
                    new ShootSequence(shooter, indexer, drivetrain, limelight, indexCommand, interpolationTable)     // Shoot the 2 indexed balls (collects both)
                ),
                sequence(
